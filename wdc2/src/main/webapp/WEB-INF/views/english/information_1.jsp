		...
		<span style="visibility:hidden;">&nbsp;获取中</span>
			<nav aria-label="breadcrumb">
  			<ol class="breadcrumb" style="background:#0d469f ">
		 		<li  class="breadcrumb-item active"><a id="information_title" href="#" onclick="go5('')" style="color: rgba(255, 255, 255, 0.52)">Information</a></li>
				<li   class="breadcrumb-item " aria-current="page "> <a id="qkl_title" onclick="go6('')" href="#" >Blockchain Knowledge</a></li>
  			</ol>
			</nav>
...
$(document).ready(function () {
		...
		$.ajax({
			...
			success:function(result) {
				...
				$.each(result.extend.pageInfo.list,function(index,item){
					...
					var d3='</div><div class="media-body">';
					var a1='<a href="#" onclick="go1('+item.id+' )"> <h4 class="media-heading" >'+item.title+'</h4></a>';
					var p1='<p style="color:rgba(255, 255, 255, 0.52) ">'+item.author+'/'+'published in'+item.time+'</p>';
					var hr1='</div></div><hr style="border-top: 1px solid rgba(57,100,208,1);">';
					var v1=d1+d2+img1+d3+a1+p1+hr1;
					//document.getElementById("information_1").innerHTML=v1;
					$("#information_1").append(v1);
	...
	function go1(ttt){
		...
		window.location.href="${pageContext.servletContext.contextPath}/english/informationDetail_1/?id="+ttt ;
	}
</script>
