	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header" enctype="multipart/form-data">
	        ...
	        <h4 class="modal-title">产品详情修改</h4>
	      </div>
	      <div class="modal-body">
	        <form class="form-horizontal" enctype="multipart/form-data" id="myForm2" >
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      	<p class="form-control-static" id="productName_update_static"></p>
			    </div>
			  </div>
			  <div class="form-group">
			    <label class="col-sm-2 control-label">图片上传</label>
			    <div class="col-sm-10">
			      <input type="file" name="productFile"  class="form-control" id="productFile_add_input">
			  ...
			  <div class="form-group">
		    		...
		    		<div class="col-sm-10">
		      		<input type="text" name="title" class="form-control" id="title_product" >
		    	</div>
		  	</div>
		  	<div class="form-group">
		   		 <label class="col-sm-2 control-label">描述</label>
		   			 <div class="col-sm-10">
		   		   <textarea class="form-control" rows="3" name="descr" id="descr_product"></textarea>  
		    	</div>
		  	</div>
			...
			<div class="form-group">
		    		...
		    		<div class="col-sm-10">
		      		<input type="text" name="address" class="form-control" id="address_product" >
		    	</div>
		  	</div>
	      ...
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-primary" id="product_update_btn">更新</button>
	      </div>
	    </div>
	  </div>
	</div>
	...
	<div class="modal fade" id="productAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" id="product_save_btn">提交</button>
	      </div>
	    </div>
	  </div>
	</div>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-4 col-md-offset-8">
				<button class="btn btn-primary" id="product_add_modal_btn">新增</button>
				<button class="btn btn-danger" id="product_delete_all_btn">删除</button>
			</div>
		</div>
<!-- 显示表格数据 -->
		<div class="row">
...
//单查询 函数
		function getEmp(id){
			$.ajax({
				url:"${APP_PATH}/product/"+id,
				type:'GET',
				success:function(result){
					//console.log(result);
					
					var empData = result.extend.product;
					$("#productName_update_static").text(empData.id);
					$("#title_product").val(empData.title);
					$("#descr_product").text(empData.descr);
					$("#address_product").val(empData.address);
				}
			});
		}
...
//更新按钮
		$("#product_update_btn").click(function(){
			...
			//2、发送ajax请求
			$.ajax({
				url:"${APP_PATH}/product2/"+$(this).attr("edit-id"),
				type:'post',
				data:formData2,
				contentType:false,
				processData:false,
...
//批量删除
		$("#product_delete_all_btn").click(function(){
			...
			if(confirm("确认删除【"+empNames+"】吗？")){
				//发送ajax请求删除
				$.ajax({
					url:"${APP_PATH}/product/"+del_idstr,
					type:"DELETE",
					success:function(result){
						alert(result.msg);
						//回到当前页面
